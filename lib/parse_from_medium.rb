...
class Medium 
  ...
  def last_post
    last_post = {
      medium_url: @homepage.search('div.h a')[2].attribute('href').value,
           title: @homepage.xpath("//h1")[1].content
    }

    last_post[:body] = normalize(Nokogiri::HTML(open(last_post[:medium_url])).search('div.section-inner'))
  ...
  private
    def normalize post
      ...
      segments.search('figure').each { |fig| normalize_img_block(fig) }
      segments.search('a em').remove
      segments
    end

    def normalize_img_block figure
